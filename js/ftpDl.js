/*----------------------------------------------------------
	                          JavaScript du test ftp download
	------------------------------------------------------------*/
...
/*----------------------------------------------------------
	...
	------------------------------------------------------------*/
function table(perl_data, date, time,d3)
{
...
{
while((lines[lines.length-2].substring(j,j+1)!=" ") &&(j<lines[lines.length-2].length))
{
volume+=lines[lines.length-2].substring(j,j+1);
j++;
}
...
if (succ==true)
{
$("#resDetail").append("<tr><td>"+(cpt+1)+"</td><td>Success</td><td>"+temps+"</td><td>"+debit+"</td><td>"+volume+"</td><td>"+date+" "+time+"</td></tr>");
...
{
$("#resDetail").append("<tr><td>"+(cpt+1)+"</td><td>Failed</td><td>"+temps+"</td><td>"+debit+"</td><td>"+volume+"</td><td>"+date+" "+time+"</td></tr>");
...
while ((data.substring(j,j+1)!="\t")&&(j<data.length))
{
...
if ((dlfinished==true)&&(cpt==nombre)) pourcentage = 100;
...
var file;
if ($('#OptionM5').is(':checked')) file = "1M";
else if ($('#OptionM100').is(':checked')) file = "100M";
else if ($('#OptionG1').is(':checked')) file = "1G";
  var request3 = $.ajax({
        type: "GET",
        url: "perl/size.pl",
	data :  {"file": file},
...
if (OptionChemin==true){
 ...
 var request7 = $.ajax({
        type: "POST",
        url: "perl/sizeOther.pl",
	data :  {"lab": lab , "OptionH1": OptionH1, "OptionH1Text" : OptionH1Text , "OptionH2": OptionH2, "OptionH2Text" : OptionH2Text , "OptionCheminText": OptionCheminText},
...
if ($('#OptionM5').is(':checked')) tot = 1*(1048576);
